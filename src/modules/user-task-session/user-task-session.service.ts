...
export class UserTaskSessionService {
  ...
  async findByUserIdAndTaskId(
    ...
    return await this.userTaskSessionSessionModel
      .find({userId: userId, taskId: taskId})
      .exec();
  }

  async removeByUserIdAndTaskId(userId: string, taskId: string) {
    return await this.userTaskSessionSessionModel
      .findOneAndDelete({userId: userId, taskId: taskId})
      .exec();
  }
  ...
  async update(
    ...
    return await this.userTaskSessionSessionModel.findByIdAndUpdate(
      id,
      userTaskSession,
      {
        new: true,
    ...
    while (attempt < 2) {
      try {
        if (userTaskSession) {
          ...
          const result =
            await this.userTaskSessionSessionModel.findByIdAndUpdate(
              id,
              userTaskSession,
              {
                new: true,
        ...
        attempt++;
        console.error(
          `Error opening modal ${rank}: ${openPageDto.title}: ${
            openPageDto.url
          } ${new Date()}`,
        );
  ...
  async closePage(
    ...
    while (attempt < 2) {
      try {
        if (userTaskSession) {
          userTaskSession.pages[rank][
            userTaskSession.pages[rank].length - 1
          ].endTime = new Date();
          const result =
            await this.userTaskSessionSessionModel.findByIdAndUpdate(
              id,
